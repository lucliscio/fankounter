<?php
   /*
    * engine.class.php
    *                                       __       HZKnight free PHP Scripts    _    vs 5.1
    *                                      / _| __ _ _ __   /\ /\___  _   _ _ __ | |_ ___ _ __
    *                                     | |_ / _` | '_ \ / //_/ _ \| | | | '_ \| __/ _ \ '__|
    *                                     |  _| (_| | | | / __ \ (_) | |_| | | | | ||  __/ |
    *                                     |_|  \__,_|_| |_\/  \/\___/ \__,_|_| |_|\__\___|_|
    *
    *                                           lucliscio <lucliscio@h0model.org>, ITALY
    *
    * -------------------------------------------------------------------------------------------
    * Documentazione di riferimento
    * -------------------------------------------------------------------------------------------
    * license.txt - le condizioni di utilizzo, modifica e redistribuzione per l'utente finale
    *  manual.txt - la guida alla configurazione, all'installazione e all'uso dello script
    *    faqs.txt - le risposte alle domande più comuni, sui problemi e sulle funzionalità
    * history.txt - la progressione delle versioni, i miglioramenti apportati e i bugs eliminati
    *
    * -------------------------------------------------------------------------------------------
    * Licence
    * -------------------------------------------------------------------------------------------
    * Copyright (C)2024 HZKnight
    *
    * This program is free software: you can redistribute it and/or modify
    * it under the terms of the GNU Affero General Public License as published by
    * the Free Software Foundation, either version 3 of the License, or
    * (at your option) any later version.
    *
    * This program is distributed in the hope that it will be useful,
    * but WITHOUT ANY WARRANTY; without even the implied warranty of
    * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    * GNU Affero General Public License for more details.
    *
    * You should have received a copy of the GNU Affero General Public License
    * along with this program.  If not, see <http://www.gnu.org/licenses/agpl-3.0.html>.
    */

    /**
     *  Modulo per il rilevamanto dei motori di ricerca.
     *
     *  @author  lucliscio <lucliscio@h0model.org>
     *  @version v 1.0
     *  @copyright Copyright 2024 HZKnight
     *  @license http://www.gnu.org/licenses/agpl-3.0.html GNU/AGPL3
     *
     *  @package fanKounter
     *  @filesource
     */

    class Engine {

        public function isSearchEngine ($url): array {
            global $inf__engine;

            if(filter_var($url, FILTER_VALIDATE_URL)){
                foreach($inf__engine as $__name=>$data){

                    $__reg = $data['reg'];

                    $host = parse_url($url)['host'];
                    
                    if(preg_match_all($__reg, $host, $matches, PREG_SET_ORDER, 0)){

                        $name = ucfirst($__name);

                        $lang = $this->getLanguage($host, $data['ext']);
                        
                        if($lang != ""){
                            $name = $name." ".$lang;
                        }

                        return array(
                            "status" => true,
                            "name" => $name
                        );
                    }
                }
            }

            return array(
                "status" => false,
                "name" => ""
            );
        }

        public function getLanguage($host, $dext): string {
            global $inf__country;

            $exhost = explode(".",$host);
            $ext = $exhost[count($exhost)-1];
            
            if(array_key_exists($ext, $inf__country)){
                if($ext != $dext){
                    return $inf__country[$ext];
                }
            } elseif(array_key_exists($exhost[0], $inf__country)){
                return $inf__country[$exhost[0]];
            }

            return "";
        }

    }
    